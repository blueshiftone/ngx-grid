...
export const nodes = {
  ...
  image: {
    ...
    toDOM(node: ProseMirrorNode) { let {src, alt, title} = node.attrs; return ["img", {src, alt, title}] }
  },
...
const emDOM = ["em", 0], underlineDOM = ["u", 0], strikeDOM = ["s", 0], strongDOM = ["strong", 0], codeDOM = ["code", 0]
...
export const marks = {
  ...
  link: {
    ...
    toDOM(node: ProseMirrorNode) { let {href, title} = node.attrs; return ["a", {href, title, target: '_blank'}, 0] }
  },
...
const nodeExport = (nodes as any) as  {
  doc            : NodeSpec;
  paragraph      : NodeSpec;
  blockquote     : NodeSpec;
...
const markExport = (marks as any) as {
  link  : MarkSpec;
  em    : MarkSpec;
  strike: MarkSpec;
  strong: MarkSpec;
